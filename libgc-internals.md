...
指すように見えるか」だけを総当たりで調べます。これこそが、[「保守的GCとは何か」の章](what-is-conservative.md)で述べた「あいまいなルート」の正体です。意味を問わないからこそ、コンパイラ
の協力なしに動けるのです。
...
ることで、一度に長く止まることを避けています。裏を返せば、スイープの仕事が
個々の確保処理に混ざるので、確保がそのぶん少し重くなることがあります。
停止時間のかたまりを、確保コストの分散に変換する技法だと言えます。
...
(blacklisting)** という手法で対抗します [](#cite:boehm1993)。
